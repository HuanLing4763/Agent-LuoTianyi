    ...
    def add(raw, target):
        ...
        if title and title not in seen:
            ...
            if href.startswith(("https://", "http://")):
                ...
                target = href.split("#", 1)[0].replace("_", " ").strip()
...
def _fields(template, missing):
    ...
    for param in template.params:
        ...
        staff_param = staff and (
            folded.startswith(rule["list_prefix"])
            or folded in rule["exclude_params"]
            or key.isdigit()
        )
        if is_presentation_key(normkey) or (not staff and is_excluded_field(normkey)) or staff_param:
            continue
        ...
        for node in code.nodes:
            ...
            if isinstance(node, Tag):
                ...
                if table is not None:
                    ...
                    adjacent_section = (section == "简介" and tag == "div") or (
                        section == "歌词" and not table_done
                    )
                    if boxes <= 1 and (main_table or adjacent_section):
