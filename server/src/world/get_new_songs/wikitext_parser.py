...
# Songbox 中只影响展示、不属于业务字段的参数键。
_DISPLAY_KEYS = frozenset({
    "image", "图片", "width", "图片大小", "style", "class", "color", "colour",
    "background", "背景色", "border", "state", "title", "caption",
})
...
def parse_details(
    ...
    for template in songboxes:
        for param in template.params:
            ...
            if key.casefold() in _DISPLAY_KEYS:
                continue
